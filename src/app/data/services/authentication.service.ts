...
export class AuthenticationService {
  ...
  onceUserRegistered(
    ...
    return this.http
      .post<void>(`${baseUrl}/register`, userRegistration, {
        withCredentials: true,
      })
      .pipe(
        map((_): RegistrationResponse => 'SUCCESS'),
        catchError(
          (error: HttpErrorResponse): Observable<RegistrationResponse> => {
            switch (error.status) {
              case 409:
  ...
  onceUserLoggedIn(loginRequest: LoginRequest): Observable<LoginResponse> {
    return this.http
      .post<void>(`${baseUrl}/login`, loginRequest, { withCredentials: true })
      .pipe(
  ...
  onceAccountVerified(
    ...
    return this.http
      .get<void>(`${baseUrl}/email/validate/${validationCode}`, {
        withCredentials: true,
      })
      .pipe(
        map((_): VerificationResponse => 'SUCCESS'),
        catchError(
          (error: HttpErrorResponse): Observable<VerificationResponse> => {
            switch (error.status) {
              case 400:
